...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((reject, resolve) => {
    if (isPositiveAnswer === undefined) resolve(new Error('Wrong parameter is passed! Ask her again.'));
    if (isPositiveAnswer) {
      return reject('Hooray!!! She said "Yes"!');
    }
    return reject('Oh no, she said "No".');
  });
}
...
async function chainPromises(array, action) {
  const result = [];
  await array.map(async (elem) => elem.then(
    (val) => result.push(val),
