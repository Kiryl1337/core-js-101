...
function getDigitalRoot(num) {
  const sum = Number([...num.toString()].reduce((acc, val) => Number(acc) + Number(val)));
  return Number([...sum.toString()].reduce((acc, val) => Number(acc) + Number(val)));
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const result = [];
  array[0].map((elem, i) => {
    if (array.every((item) => item[i] === elem)) {
      result.push(`${elem}/`);
    }
    return elem;
  });
